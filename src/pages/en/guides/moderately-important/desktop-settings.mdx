...
- [macOS Tahoe 26 Recommended Settings](#macos-tahoe-26-recommended-settings)
- [Windows 11 Recommended Settings](#windows-11-recommended-settings)
- [Best Practices](#best-practices)

## Linux

Just like cell phones, <dfn title="Desktop and laptop computers - compared to 'mobile' devices like phones or tablets">desktop</dfn> operating systems like Windows and Mac track their users to an excessive degree. Windows is by far the worst offender and getting worse with things like [excessive analytics](https://www.extremetech.com/computing/342941-windows-11-collects-an-awful-lot-of-telemetry-about-your-pc), [invasive AI](https://www.bleepingcomputer.com/news/microsoft/microsofts-new-windows-11-recall-is-a-privacy-nightmare/), and Microsoft aggressively enforcing [online accounts](https://www.tomshardware.com/software/windows/microsoft-clamping-down-on-windows-11-local-account-setup-latest-insider-build-removes-local-only-commands-skipping-microsoft-account-sign-in-will-crash-setup-process) that enable further data collection. However, Apple users should be aware that Mac is not 100% private either, as they also enforce an online account and collect analytics (though far less than Windows does).

**In a perfect world, the best option for a desktop operating system is [Linux](https://io9.gizmodo.com/getting-started-with-linux-the-complete-guide-5778882).** Linux is an open-source operating system with dozens of variants (called "distros," short for "distributions"), each offering their own unique set of features and target audience. Most linux distros are considerably more private compared to Windows and Mac, though some place additional emphasis on privacy or security.
...
You can also test out a number of popular distros right in your browser without installing anything using [DistroSea](https://distrosea.com/).
...
- **Macs have better privacy and security than [Windows](https://www.privacyguides.org/en/os/windows/#privacy-notes)** (though it does still collect some telemetry, which can be reduced through the recommendations on this page).
- **It is far easier to dualboot Linux with Windows** since Linux support on Apple devices is extremely limited.
- **Windows is better for gaming than Mac, but Linux is quickly catching up.** Gaming on Linux has [progressed substantially](https://www.xda-developers.com/linux-runs-most-pc-games-now-and-you-dont-need-steamos-to-do-it/) in recent years and you may find most or all of your favorite games run with few or no issues on Linux. You can use resources like [ProtonDB](https://www.protondb.com/), [The Linux Experiment](https://www.youtube.com/@TheLinuxEXP/search?query=gaming), and [GamingOnLinux](https://www.gamingonlinux.com/) for more information about what games work and the current state of gaming on Linux.

For those who must use Mac or Windows - dual booted or otherwise - I have listed a set of recommend settings for both operating systems that I encourage you to change (if you can) to make your device a little more private and secure. You can see my criteria for this page and why I recommended these settings [here](/criteria#desktop-settings--apps).

## macOS Tahoe 26 Recommended Settings
**_NOTE:_** _Different Mac devices may have additional or slightly different options. These settings are based on the 2025 Macbook Air._

<Highlighting>
**During setup:**
- If you are setting up a new device from scratch, you should be able to continue through setup without entering an Apple ID. You can get all of the programs I recommend without using the App Store, and major system updates will still be applied even without signing in. However, your apps will not auto-update in this situation.
</Highlighting>
- Wi-Fi > Details (next to your current network) > Private Wi-Fi address: Rotating
...
- Wi-Fi > Advanced (at the bottom) > Remove old "known networks" you no longer use regularly
- Bluetooth: Turn off whenever not in use
- Network > Firewall > Firewall: Enable (Note the warning about how, by default, the firewall blocks sharing services and adjust accordingly if needed)
- Battery > Options > Wake for network access: Never
- General > Automatic Updates: Enable all (click the "i")
- General > AirDrop: No One (Only enable as-needed)
- General > AirPlay > AirPlay Receiver: Disabled
- General > AirPlay > Allow Airplay for: Current User
- General > AirPlay > Require Password: Enabled
- General > AutoFill & Passwords: Select one of our recommended [password managers](/guides/most-important/passwords)
- General > Date & Time > Source > Set: pool.ntp.org
- General > Sharing: Disable all
- General > Time Machine: See [this page](/guides/less-important/backups) for more information about how to keep good backups
- Accessibility > Siri > Type to Siri: Disabled
- Accessibility > Siri > Listen for atypical speech: Disabled
- Apple Intelligence & Siri > Apple Intelligence: Apple claims to have implemented this in a way that's privacy respecting but be sure to disable the ChatGPT extension. Using ChatGPT will forward some of your data to OpenAI and is not covered by Apple's privacy guarantees. (If you need a standalone LLM, I encourage you to consider one from [this list](/guides/less-important/misc#ai) instead.)
- Apple Intelligence & Siri > Siri: disable
- Apple Intelligence & Siri > Siri > Delete Siri & Dictation History
- Apple Intelligence & Siri > Siri Suggestions & Privacy: Disable all
- Spotlight > Show Related Content: disable
...
- Spotlight > Results from Clipboard: disable
- Notificatoins > Show previews: "When Unlocked" or "Never"
- Notifications > Show Notifications > When the display is sleeping: Off
- Notifications > Show Notifications > When the screen is locked: Off
- Notifications > Show Notifications > When mirroring or sharing the display: Notifications Off
- Notifications > Application Notifications: Disable any you don't need or want
- Focus > Share across devices: Disabled
- Focus > Focus status > Off
- Lock Screen > Set "Turn display off when inactive" to shortest duration you're comfortable with
...
- Privacy & Security > Security > Allow apps downloaded from: App Store and identified developers
...
- Privacy & Security > Background Security Improvements > Automatically Install: Enabled
- Privacy & Security > Advanced > Require an administrator password to access systyem-wide settings: Enabled
- Privacy & Security > Advanced > Log out automatically after inactivity: Enabled
- Touch ID & Password: A fingerprint is acceptable if your device allows it. Face ID should be avoided.
- iCloud: I strongly discourage the use of iCloud as I believe most of its benefits can be replicated in a more privacy-respecting way using other services listed on this site. However, if you wish to use iCloud, enable [Advanced Data Protection](/guides/less-important/backups/#iclouds-advanced-data-protection-program).
- Wallet & Apple Pay: See [here](/guides/moderately-important/payments/)
