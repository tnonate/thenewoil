...
**For maximum privacy and security, it is imperative to use a specific type of encryption called “End-to-End Encryption"** (also known by other names such as "e2ee," “zero-knowledge encryption”, "zero-access encryption," or "client-side"). This means that data is encrypted in-transit from sender to reciever - not decrypted at the provider's servers. Only the sender and recipient can read the data, nobody else in between can - not even the provider. (See the caveat under "[Tips & Tricks](#tips--tricks)")
...
- Don't be swayed by jargon like "military grade encryption." This is a meaningless marketing term that has no value or information in it. Check the [privacy policy](https://write.as/thenewoil/how-to-read-a-privacy-policy) to see what data the service can really access.

- Be aware that terms like "zero access" or "zero knowledge" can sometimes be misleading as these services may not always necessarily be truly ZERO access/knowledge. While the content itself may be encrypted, there may still be metadata or other peripheral data that is accessible. Be sure to check the privacy policy carefully - and contact the service if necessary - to understand what's truly protected and what isn't.
