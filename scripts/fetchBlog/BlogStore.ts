...
const sortByMostRecentPost = (postA: Post, postB: Post) => compareDate(
...
class BlogStore {
    ...
    async GetBlogMeta(): Promise<CollectionMeta> {
        const fileHandle = await fs.open(this.config.metaStoragePath, "a+" /* Opens/creates file for reading */);
    ...
    async SaveBlogMeta(meta: CollectionMeta) {
        const fileHandle = await fs.open(this.config.metaStoragePath, "w+" /* Opens/creates file for writing */);
        await fileHandle.writeFile(JSON.stringify(meta));
        await fileHandle.close();
    }

    async GetBlogPosts(): Promise<Post[]> {
        const fileHandle = await fs.open(this.config.postsStoragePath, "a+" /* Opens/creates file for reading */);
        const fileContent = (await fileHandle.readFile()).toString();
        await fileHandle.close();
    ...
    async SaveBlogPosts(posts: Post[]) {
        ...
        const fileHandle = await fs.open(this.config.postsStoragePath, "w+" /* Opens/creates file for writing */);
