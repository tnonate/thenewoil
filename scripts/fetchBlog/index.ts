...
const blogStore = new BlogStore({
    metaStoragePath: path.join(process.cwd(), "src", "assets", "data", "blog-index", "blog-meta.json"),
    postsStoragePath: path.join(process.cwd(), "src", "assets", "data", "blog-index", "blog-pages.json")
});
...
if (storedPosts.length > storedMeta.total_posts) {
    ...
    storedPosts = storedMeta.total_posts === 0 ? [] : storedPosts.slice(sliceStart);
}
...
if (deltaTotalPosts > 0) {
    ...
    const fetchedPosts = await new Promise<Post[]>((resolve, reject) => {
        ...
        const createFetchTask = async () => {
            ...
            const response = await writeAsClient.FetchCollectionPage(COLLECTION_NAME, pageNumber);

            if (typeof response === "number") {
                if (response === 429) {
                    console.log("Write.as has rate limited the fetch-script, saving current posts. Restart the script later.");
                }
    ...
    let updatedPosts = storedPosts
        .filter((storedPost) => fetchedPosts.findIndex((fetchedPost) => fetchedPost.id === storedPost.id) === -1);
